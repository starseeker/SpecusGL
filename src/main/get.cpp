...
/**
 * \file get.cpp
 * Implementation of glGetBooleanv, glGetFloatv, glGetIntegerv, glGetDoublev.
 */
...
[[nodiscard]] static constexpr GLfloat ENUM_TO_FLOAT(GLenum e) noexcept { return static_cast<GLfloat>(e); }
[[nodiscard]] static constexpr GLint BOOLEAN_TO_INT(GLboolean b) noexcept { return static_cast<GLint>(b); }
[[nodiscard]] static constexpr GLfloat BOOLEAN_TO_FLOAT(GLboolean b) noexcept { return b ? 1.0F : 0.0F; }
...
#define CHECK_EXT1(EXT1, FUNC)                                         \
   if (!ctx->Extensions.EXT1) {                                        \
      _mesa_error(ctx, GL_INVALID_ENUM, FUNC "(0x%x)", static_cast<int>(pname));  \
      return;                                                          \
   }
...
_mesa_GetBooleanv(GLenum pname, GLboolean *params)
{
	...
	case GL_VERTEX_PROGRAM_BINDING_NV:
	    ...
	    params[0] = INT_TO_BOOLEAN((ctx->VertexProgram.Current ? ctx->VertexProgram.Current->Id : 0));
	    break;
	...
	case GL_FRAGMENT_PROGRAM_BINDING_NV:
	    CHECK_EXT1(NV_fragment_program, "GetBooleanv");
	    params[0] = INT_TO_BOOLEAN(ctx->FragmentProgram.Current ? ctx->FragmentProgram.Current->Id : 0);
	    break;
...
_mesa_GetFloatv(GLenum pname, GLfloat *params)
{
    ...
    switch (pname) {
	case GL_ACCUM_RED_BITS:
	    params[0] = static_cast<GLfloat>(ctx->DrawBuffer->Visual.accumRedBits);
	    break;
	case GL_ACCUM_GREEN_BITS:
	    params[0] = static_cast<GLfloat>(ctx->DrawBuffer->Visual.accumGreenBits);
	    break;
	case GL_ACCUM_BLUE_BITS:
	    params[0] = static_cast<GLfloat>(ctx->DrawBuffer->Visual.accumBlueBits);
	    break;
	case GL_ACCUM_ALPHA_BITS:
	    params[0] = static_cast<GLfloat>(ctx->DrawBuffer->Visual.accumAlphaBits);
	    break;
	case GL_ACCUM_CLEAR_VALUE:
	...
	case GL_ALPHA_BIAS:
	    ...
	    break;
	case GL_ALPHA_BITS:
	    params[0] = static_cast<GLfloat>(ctx->DrawBuffer->Visual.alphaBits);
	    break;
	case GL_ALPHA_SCALE:
	...
	case GL_ALPHA_TEST_REF:
	    ...
	    break;
	case GL_ATTRIB_STACK_DEPTH:
	    params[0] = static_cast<GLfloat>(static_cast<GLuint>(ctx->AttribStack.size()));
	    break;
	case GL_AUTO_NORMAL:
	    params[0] = BOOLEAN_TO_FLOAT(ctx->Eval.AutoNormal);
	    break;
	case GL_AUX_BUFFERS:
	    params[0] = static_cast<GLfloat>(ctx->DrawBuffer->Visual.numAuxBuffers);
	    break;
	case GL_BLEND:
	...
	case GL_BLUE_BIAS:
	    ...
	    break;
	case GL_BLUE_BITS:
	    params[0] = static_cast<GLfloat>(ctx->DrawBuffer->Visual.blueBits);
	    break;
	case GL_BLUE_SCALE:
	    params[0] = ctx->Pixel.BlueScale;
	    break;
	case GL_CLIENT_ATTRIB_STACK_DEPTH:
	    params[0] = static_cast<GLfloat>(static_cast<GLuint>(ctx->ClientAttribStack.size()));
	    break;
	case GL_CLIP_PLANE0:
	...
	case GL_COLOR_MATERIAL_PARAMETER:
	    ...
	    break;
	case GL_COLOR_WRITEMASK:
	    params[0] = static_cast<GLfloat>(ctx->Color.ColorMask[RCOMP] ? 1 : 0);
	    params[1] = static_cast<GLfloat>(ctx->Color.ColorMask[GCOMP] ? 1 : 0);
	    params[2] = static_cast<GLfloat>(ctx->Color.ColorMask[BCOMP] ? 1 : 0);
	    params[3] = static_cast<GLfloat>(ctx->Color.ColorMask[ACOMP] ? 1 : 0);
	    break;
	case GL_CULL_FACE:
	...
	case GL_DEPTH_BIAS:
	    ...
	    break;
	case GL_DEPTH_BITS:
	    params[0] = static_cast<GLfloat>(ctx->DrawBuffer->Visual.depthBits);
	    break;
	case GL_DEPTH_CLEAR_VALUE:
	...
	break;
	case GL_FEEDBACK_BUFFER_SIZE:
	    params[0] = static_cast<GLfloat>(ctx->Feedback.BufferSize);
	    break;
	case GL_FEEDBACK_BUFFER_TYPE:
	...
	case GL_GREEN_BIAS:
	    ...
	    break;
	case GL_GREEN_BITS:
	    params[0] = static_cast<GLfloat>(ctx->DrawBuffer->Visual.greenBits);
	    break;
	case GL_GREEN_SCALE:
	    params[0] = ctx->Pixel.GreenScale;
	    break;
	case GL_INDEX_BITS:
	    params[0] = static_cast<GLfloat>(ctx->DrawBuffer->Visual.indexBits);
	    break;
	case GL_INDEX_CLEAR_VALUE:
	    params[0] = static_cast<GLfloat>(ctx->Color.ClearIndex);
	    break;
	case GL_INDEX_MODE:
	    params[0] = BOOLEAN_TO_FLOAT(!ctx->DrawBuffer->Visual.rgbMode);
	    break;
	case GL_INDEX_OFFSET:
	    params[0] = static_cast<GLfloat>(ctx->Pixel.IndexOffset);
	    break;
	case GL_INDEX_SHIFT:
	    params[0] = static_cast<GLfloat>(ctx->Pixel.IndexShift);
	    break;
	case GL_INDEX_WRITEMASK:
	    params[0] = static_cast<GLfloat>(ctx->Color.IndexMask);
	    break;
	case GL_LIGHT0:
	...
	case GL_LINE_STIPPLE:
	    ...
	    break;
	case GL_LINE_STIPPLE_PATTERN:
	    params[0] = static_cast<GLfloat>(ctx->Line.StipplePattern);
	    break;
	case GL_LINE_STIPPLE_REPEAT:
	    params[0] = static_cast<GLfloat>(ctx->Line.StippleFactor);
	    break;
	case GL_LINE_WIDTH:
	...
	case GL_ALIASED_LINE_WIDTH_RANGE:
	    ...
	    break;
	case GL_LIST_BASE:
	    params[0] = static_cast<GLfloat>(ctx->List.ListBase);
	    break;
	case GL_LIST_INDEX:
	    params[0] = static_cast<GLfloat>(ctx->ListState.CurrentListNum);
	    break;
	case GL_LIST_MODE: {
	    GLenum mode;
	    if (!ctx->CompileFlag)
	...
	case GL_MAP1_GRID_DOMAIN:
	    ...
	    break;
	case GL_MAP1_GRID_SEGMENTS:
	    params[0] = static_cast<GLfloat>(ctx->Eval.MapGrid1un);
	    break;
	case GL_MAP1_INDEX:
	...
	case GL_MAP2_GRID_DOMAIN:
	    ...
	    break;
	case GL_MAP2_GRID_SEGMENTS:
	    params[0] = static_cast<GLfloat>(ctx->Eval.MapGrid2un);
	    params[1] = static_cast<GLfloat>(ctx->Eval.MapGrid2vn);
	    break;
	case GL_MAP2_INDEX:
	...
	case GL_MATRIX_MODE:
	    ...
	    break;
	case GL_MAX_ATTRIB_STACK_DEPTH:
	    params[0] = static_cast<GLfloat>(MAX_ATTRIB_STACK_DEPTH);
	    break;
	case GL_MAX_CLIENT_ATTRIB_STACK_DEPTH:
	    params[0] = static_cast<GLfloat>(MAX_CLIENT_ATTRIB_STACK_DEPTH);
	    break;
	case GL_MAX_CLIP_PLANES:
	    params[0] = static_cast<GLfloat>(ctx->Const.MaxClipPlanes);
	    break;
	case GL_MAX_ELEMENTS_VERTICES:
	    params[0] = static_cast<GLfloat>(ctx->Const.MaxArrayLockSize);
	    break;
	case GL_MAX_ELEMENTS_INDICES:
	    params[0] = static_cast<GLfloat>(ctx->Const.MaxArrayLockSize);
	    break;
	case GL_MAX_EVAL_ORDER:
	    params[0] = static_cast<GLfloat>(MAX_EVAL_ORDER);
	    break;
	case GL_MAX_LIGHTS:
	    params[0] = static_cast<GLfloat>(ctx->Const.MaxLights);
	    break;
	case GL_MAX_LIST_NESTING:
	    params[0] = static_cast<GLfloat>(MAX_LIST_NESTING);
	    break;
	case GL_MAX_MODELVIEW_STACK_DEPTH:
	    params[0] = static_cast<GLfloat>(MAX_MODELVIEW_STACK_DEPTH);
	    break;
	case GL_MAX_NAME_STACK_DEPTH:
	    params[0] = static_cast<GLfloat>(MAX_NAME_STACK_DEPTH);
	    break;
	case GL_MAX_PIXEL_MAP_TABLE:
	    params[0] = static_cast<GLfloat>(MAX_PIXEL_MAP_TABLE);
	    break;
	case GL_MAX_PROJECTION_STACK_DEPTH:
	    params[0] = static_cast<GLfloat>(MAX_PROJECTION_STACK_DEPTH);
	    break;
	case GL_MAX_TEXTURE_SIZE:
	    params[0] = static_cast<GLfloat>(1 << (ctx->Const.MaxTextureLevels - 1));
	    break;
	case GL_MAX_3D_TEXTURE_SIZE:
	    params[0] = static_cast<GLfloat>(1 << (ctx->Const.Max3DTextureLevels - 1));
	    break;
	case GL_MAX_TEXTURE_STACK_DEPTH:
	    params[0] = static_cast<GLfloat>(MAX_TEXTURE_STACK_DEPTH);
	    break;
	case GL_MAX_VIEWPORT_DIMS:
	    params[0] = static_cast<GLfloat>(ctx->Const.MaxViewportWidth);
	    params[1] = static_cast<GLfloat>(ctx->Const.MaxViewportHeight);
	    break;
	case GL_MODELVIEW_MATRIX: {
	    const GLfloat *matrix = ctx->ModelviewMatrixStack.Top->m;
	    params[0] = matrix[0];
	...
	break;
	case GL_MODELVIEW_STACK_DEPTH:
	    params[0] = static_cast<GLfloat>(ctx->ModelviewMatrixStack.Depth + 1);
	    break;
	case GL_NAME_STACK_DEPTH:
	    params[0] = static_cast<GLfloat>(ctx->Select.NameStackDepth);
	    break;
	case GL_NORMALIZE:
	    params[0] = BOOLEAN_TO_FLOAT(ctx->Transform.Normalize);
	    break;
	case GL_PACK_ALIGNMENT:
	    params[0] = static_cast<GLfloat>(ctx->Pack.Alignment);
	    break;
	case GL_PACK_LSB_FIRST:
	    params[0] = BOOLEAN_TO_FLOAT(ctx->Pack.LsbFirst);
	    break;
	case GL_PACK_ROW_LENGTH:
	    params[0] = static_cast<GLfloat>(ctx->Pack.RowLength);
	    break;
	case GL_PACK_SKIP_PIXELS:
	    params[0] = static_cast<GLfloat>(ctx->Pack.SkipPixels);
	    break;
	case GL_PACK_SKIP_ROWS:
	    params[0] = static_cast<GLfloat>(ctx->Pack.SkipRows);
	    break;
	case GL_PACK_SWAP_BYTES:
	    params[0] = BOOLEAN_TO_FLOAT(ctx->Pack.SwapBytes);
	    break;
	case GL_PACK_SKIP_IMAGES_EXT:
	    params[0] = static_cast<GLfloat>(ctx->Pack.SkipImages);
	    break;
	case GL_PACK_IMAGE_HEIGHT_EXT:
	    params[0] = static_cast<GLfloat>(ctx->Pack.ImageHeight);
	    break;
	case GL_PACK_INVERT_MESA:
	...
	case GL_PERSPECTIVE_CORRECTION_HINT:
	    ...
	    break;
	case GL_PIXEL_MAP_A_TO_A_SIZE:
	    params[0] = static_cast<GLfloat>(ctx->PixelMaps.AtoA.Size);
	    break;
	case GL_PIXEL_MAP_B_TO_B_SIZE:
	    params[0] = static_cast<GLfloat>(ctx->PixelMaps.BtoB.Size);
	    break;
	case GL_PIXEL_MAP_G_TO_G_SIZE:
	    params[0] = static_cast<GLfloat>(ctx->PixelMaps.GtoG.Size);
	    break;
	case GL_PIXEL_MAP_I_TO_A_SIZE:
	    params[0] = static_cast<GLfloat>(ctx->PixelMaps.ItoA.Size);
	    break;
	case GL_PIXEL_MAP_I_TO_B_SIZE:
	    params[0] = static_cast<GLfloat>(ctx->PixelMaps.ItoB.Size);
	    break;
	case GL_PIXEL_MAP_I_TO_G_SIZE:
	    params[0] = static_cast<GLfloat>(ctx->PixelMaps.ItoG.Size);
	    break;
	case GL_PIXEL_MAP_I_TO_I_SIZE:
	    params[0] = static_cast<GLfloat>(ctx->PixelMaps.ItoI.Size);
	    break;
	case GL_PIXEL_MAP_I_TO_R_SIZE:
	    params[0] = static_cast<GLfloat>(ctx->PixelMaps.ItoR.Size);
	    break;
	case GL_PIXEL_MAP_R_TO_R_SIZE:
	    params[0] = static_cast<GLfloat>(ctx->PixelMaps.RtoR.Size);
	    break;
	case GL_PIXEL_MAP_S_TO_S_SIZE:
	    params[0] = static_cast<GLfloat>(ctx->PixelMaps.StoS.Size);
	    break;
	case GL_POINT_SIZE:
	...
	break;
	case GL_PROJECTION_STACK_DEPTH:
	    params[0] = static_cast<GLfloat>(ctx->ProjectionMatrixStack.Depth + 1);
	    break;
	case GL_READ_BUFFER:
	...
	case GL_RED_BIAS:
	    ...
	    break;
	case GL_RED_BITS:
	    params[0] = static_cast<GLfloat>(ctx->DrawBuffer->Visual.redBits);
	    break;
	case GL_RED_SCALE:
	...
	case GL_RGBA_MODE:
	    ...
	    break;
	case GL_SCISSOR_BOX:
	    params[0] = static_cast<GLfloat>(ctx->Scissor.X);
	    params[1] = static_cast<GLfloat>(ctx->Scissor.Y);
	    params[2] = static_cast<GLfloat>(ctx->Scissor.Width);
	    params[3] = static_cast<GLfloat>(ctx->Scissor.Height);
	    break;
	case GL_SCISSOR_TEST:
	    params[0] = BOOLEAN_TO_FLOAT(ctx->Scissor.Enabled);
	    break;
	case GL_SELECTION_BUFFER_SIZE:
	    params[0] = static_cast<GLfloat>(ctx->Select.BufferSize);
	    break;
	case GL_SHADE_MODEL:
	...
	case GL_SHARED_TEXTURE_PALETTE_EXT:
	    ...
	    break;
	case GL_STENCIL_BITS:
	    params[0] = static_cast<GLfloat>(ctx->DrawBuffer->Visual.stencilBits);
	    break;
	case GL_STENCIL_CLEAR_VALUE:
	    params[0] = static_cast<GLfloat>(ctx->Stencil.Clear);
	    break;
	case GL_STENCIL_FAIL:
	...
	case GL_STENCIL_PASS_DEPTH_PASS:
	    ...
	    break;
	case GL_STENCIL_REF:
	    params[0] = static_cast<GLfloat>(ctx->Stencil.Ref[ctx->Stencil.ActiveFace]);
	    break;
	case GL_STENCIL_TEST:
	    params[0] = BOOLEAN_TO_FLOAT(ctx->Stencil.Enabled);
	    break;
	case GL_STENCIL_VALUE_MASK:
	    params[0] = static_cast<GLfloat>(ctx->Stencil.ValueMask[ctx->Stencil.ActiveFace]);
	    break;
	case GL_STENCIL_WRITEMASK:
	    params[0] = static_cast<GLfloat>(ctx->Stencil.WriteMask[ctx->Stencil.ActiveFace]);
	    break;
	case GL_STEREO:
	    params[0] = BOOLEAN_TO_FLOAT(ctx->DrawBuffer->Visual.stereoMode);
	    break;
	case GL_SUBPIXEL_BITS:
	    params[0] = static_cast<GLfloat>(ctx->Const.SubPixelBits);
	    break;
	case GL_TEXTURE_1D:
	...
	case GL_TEXTURE_3D:
	    ...
	    break;
	case GL_TEXTURE_BINDING_1D:
	    params[0] = static_cast<GLfloat>(ctx->Texture.Unit[ctx->Texture.CurrentUnit].Current1D->Name);
	    break;
	case GL_TEXTURE_BINDING_2D:
	    params[0] = static_cast<GLfloat>(ctx->Texture.Unit[ctx->Texture.CurrentUnit].Current2D->Name);
	    break;
	case GL_TEXTURE_BINDING_3D:
	    params[0] = static_cast<GLfloat>(ctx->Texture.Unit[ctx->Texture.CurrentUnit].Current3D->Name);
	    break;
	case GL_TEXTURE_GEN_S:
	...
	break;
	case GL_TEXTURE_STACK_DEPTH:
	    params[0] = static_cast<GLfloat>(ctx->TextureMatrixStack[ctx->Texture.CurrentUnit].Depth + 1);
	    break;
	case GL_UNPACK_ALIGNMENT:
	    params[0] = static_cast<GLfloat>(ctx->Unpack.Alignment);
	    break;
	case GL_UNPACK_LSB_FIRST:
	    params[0] = BOOLEAN_TO_FLOAT(ctx->Unpack.LsbFirst);
	    break;
	case GL_UNPACK_ROW_LENGTH:
	    params[0] = static_cast<GLfloat>(ctx->Unpack.RowLength);
	    break;
	case GL_UNPACK_SKIP_PIXELS:
	    params[0] = static_cast<GLfloat>(ctx->Unpack.SkipPixels);
	    break;
	case GL_UNPACK_SKIP_ROWS:
	    params[0] = static_cast<GLfloat>(ctx->Unpack.SkipRows);
	    break;
	case GL_UNPACK_SWAP_BYTES:
	    params[0] = BOOLEAN_TO_FLOAT(ctx->Unpack.SwapBytes);
	    break;
	case GL_UNPACK_SKIP_IMAGES_EXT:
	    params[0] = static_cast<GLfloat>(ctx->Unpack.SkipImages);
	    break;
	case GL_UNPACK_IMAGE_HEIGHT_EXT:
	    params[0] = static_cast<GLfloat>(ctx->Unpack.ImageHeight);
	    break;
	case GL_UNPACK_CLIENT_STORAGE_APPLE:
	    params[0] = BOOLEAN_TO_FLOAT(ctx->Unpack.ClientStorage);
	    break;
	case GL_VIEWPORT:
	    params[0] = static_cast<GLfloat>(ctx->Viewport.X);
	    params[1] = static_cast<GLfloat>(ctx->Viewport.Y);
	    params[2] = static_cast<GLfloat>(ctx->Viewport.Width);
	    params[3] = static_cast<GLfloat>(ctx->Viewport.Height);
	    break;
	case GL_ZOOM_X:
	...
	case GL_VERTEX_ARRAY:
	    ...
	    break;
	case GL_VERTEX_ARRAY_SIZE:
	    params[0] = static_cast<GLfloat>(ctx->Array.ArrayObj->Vertex.Size);
	    break;
	case GL_VERTEX_ARRAY_TYPE:
	    params[0] = ENUM_TO_FLOAT(ctx->Array.ArrayObj->Vertex.Type);
	    break;
	case GL_VERTEX_ARRAY_STRIDE:
	    params[0] = static_cast<GLfloat>(ctx->Array.ArrayObj->Vertex.Stride);
	    break;
	case GL_VERTEX_ARRAY_COUNT_EXT:
	    params[0] = static_cast<GLfloat>(0);
	    break;
	case GL_NORMAL_ARRAY:
	...
	case GL_NORMAL_ARRAY_TYPE:
	    ...
	    break;
	case GL_NORMAL_ARRAY_STRIDE:
	    params[0] = static_cast<GLfloat>(ctx->Array.ArrayObj->Normal.Stride);
	    break;
	case GL_NORMAL_ARRAY_COUNT_EXT:
	    params[0] = static_cast<GLfloat>(0);
	    break;
	case GL_COLOR_ARRAY:
	    ...
	    break;
	case GL_COLOR_ARRAY_SIZE:
	    params[0] = static_cast<GLfloat>(ctx->Array.ArrayObj->Color.Size);
	    break;
	case GL_COLOR_ARRAY_TYPE:
	    params[0] = ENUM_TO_FLOAT(ctx->Array.ArrayObj->Color.Type);
	    break;
	case GL_COLOR_ARRAY_STRIDE:
	    params[0] = static_cast<GLfloat>(ctx->Array.ArrayObj->Color.Stride);
	    break;
	case GL_COLOR_ARRAY_COUNT_EXT:
	    params[0] = static_cast<GLfloat>(0);
	    break;
	...
	case GL_INDEX_ARRAY_TYPE:
	    ...
	    break;
	case GL_INDEX_ARRAY_STRIDE:
	    params[0] = static_cast<GLfloat>(ctx->Array.ArrayObj->Index.Stride);
	    break;
	case GL_INDEX_ARRAY_COUNT_EXT:
	    params[0] = static_cast<GLfloat>(0);
	    break;
	case GL_TEXTURE_COORD_ARRAY:
	    ...
	    break;
	case GL_TEXTURE_COORD_ARRAY_SIZE:
	    params[0] = static_cast<GLfloat>(ctx->Array.ArrayObj->TexCoord[ctx->Array.ActiveTexture].Size);
	    break;
	case GL_TEXTURE_COORD_ARRAY_TYPE:
	    params[0] = ENUM_TO_FLOAT(ctx->Array.ArrayObj->TexCoord[ctx->Array.ActiveTexture].Type);
	    break;
	case GL_TEXTURE_COORD_ARRAY_STRIDE:
	    params[0] = static_cast<GLfloat>(ctx->Array.ArrayObj->TexCoord[ctx->Array.ActiveTexture].Stride);
	    break;
	case GL_TEXTURE_COORD_ARRAY_COUNT_EXT:
	    params[0] = static_cast<GLfloat>(0);
	    break;
	case GL_EDGE_FLAG_ARRAY:
	    ...
	    break;
	case GL_EDGE_FLAG_ARRAY_STRIDE:
	    params[0] = static_cast<GLfloat>(ctx->Array.ArrayObj->EdgeFlag.Stride);
	    break;
	case GL_EDGE_FLAG_ARRAY_COUNT_EXT:
	    params[0] = static_cast<GLfloat>(0);
	    break;
	case GL_MAX_TEXTURE_UNITS_ARB:
	    ...
	    params[0] = static_cast<GLfloat>(ctx->Const.MaxTextureUnits);
	    break;
	case GL_ACTIVE_TEXTURE_ARB:
	    CHECK_EXT1(ARB_multitexture, "GetFloatv");
	    params[0] = static_cast<GLfloat>(GL_TEXTURE0_ARB + ctx->Texture.CurrentUnit);
	    break;
	case GL_CLIENT_ACTIVE_TEXTURE_ARB:
	    CHECK_EXT1(ARB_multitexture, "GetFloatv");
	    params[0] = static_cast<GLfloat>(GL_TEXTURE0_ARB + ctx->Array.ActiveTexture);
	    break;
	case GL_TEXTURE_CUBE_MAP_ARB:
	...
	case GL_TEXTURE_BINDING_CUBE_MAP_ARB:
	    CHECK_EXT1(ARB_texture_cube_map, "GetFloatv");
	    params[0] = static_cast<GLfloat>(ctx->Texture.Unit[ctx->Texture.CurrentUnit].CurrentCubeMap->Name);
	    break;
	case GL_MAX_CUBE_MAP_TEXTURE_SIZE_ARB:
	    CHECK_EXT1(ARB_texture_cube_map, "GetFloatv");
	    params[0] = static_cast<GLfloat>((1 << (ctx->Const.MaxCubeTextureLevels - 1)));
	    break;
	case GL_TEXTURE_COMPRESSION_HINT_ARB:
	    CHECK_EXT1(ARB_texture_compression, "GetFloatv");
	    params[0] = static_cast<GLfloat>(ctx->Hint.TextureCompression);
	    break;
	case GL_NUM_COMPRESSED_TEXTURE_FORMATS_ARB:
	    CHECK_EXT1(ARB_texture_compression, "GetFloatv");
	    params[0] = static_cast<GLfloat>(_mesa_get_compressed_formats(ctx, nullptr, GL_FALSE));
	    break;
	case GL_COMPRESSED_TEXTURE_FORMATS_ARB:
	    CHECK_EXT1(ARB_texture_compression, "GetFloatv");
	    {
	...
	case GL_ARRAY_ELEMENT_LOCK_FIRST_EXT:
	    ...
	    params[0] = static_cast<GLfloat>(ctx->Array.LockFirst);
	    break;
	case GL_ARRAY_ELEMENT_LOCK_COUNT_EXT:
	    CHECK_EXT1(EXT_compiled_vertex_array, "GetFloatv");
	    params[0] = static_cast<GLfloat>(ctx->Array.LockCount);
	    break;
	case GL_TRANSPOSE_COLOR_MATRIX_ARB: {
	    const GLfloat *matrix = ctx->ColorMatrixStack.Top->m;
	    params[0] = matrix[0];
	...
	break;
	case GL_COLOR_MATRIX_STACK_DEPTH_SGI:
	    params[0] = static_cast<GLfloat>(ctx->ColorMatrixStack.Depth + 1);
	    break;
	case GL_MAX_COLOR_MATRIX_STACK_DEPTH_SGI:
	    params[0] = static_cast<GLfloat>(MAX_COLOR_STACK_DEPTH);
	    break;
	case GL_POST_COLOR_MATRIX_RED_SCALE_SGI:
	...
	case GL_SECONDARY_COLOR_ARRAY_STRIDE_EXT:
	    CHECK_EXT1(EXT_secondary_color, "GetFloatv");
	    params[0] = static_cast<GLfloat>(ctx->Array.ArrayObj->SecondaryColor.Stride);
	    break;
	case GL_SECONDARY_COLOR_ARRAY_SIZE_EXT:
	    CHECK_EXT1(EXT_secondary_color, "GetFloatv");
	    params[0] = static_cast<GLfloat>(ctx->Array.ArrayObj->SecondaryColor.Size);
	    break;
	case GL_CURRENT_FOG_COORDINATE_EXT:
	...
	case GL_FOG_COORDINATE_ARRAY_STRIDE_EXT:
	    CHECK_EXT1(EXT_fog_coord, "GetFloatv");
	    params[0] = static_cast<GLfloat>(ctx->Array.ArrayObj->FogCoord.Stride);
	    break;
	case GL_FOG_COORDINATE_SOURCE_EXT:
	    CHECK_EXT1(EXT_fog_coord, "GetFloatv");
	...
	case GL_SAMPLE_BUFFERS_ARB:
	    CHECK_EXT1(ARB_multisample, "GetFloatv");
	    params[0] = static_cast<GLfloat>(ctx->DrawBuffer->Visual.sampleBuffers);
	    break;
	case GL_SAMPLES_ARB:
	    CHECK_EXT1(ARB_multisample, "GetFloatv");
	    params[0] = static_cast<GLfloat>(ctx->DrawBuffer->Visual.samples);
	    break;
	case GL_RASTER_POSITION_UNCLIPPED_IBM:
	...
	case GL_VERTEX_PROGRAM_BINDING_NV:
	    ...
	    params[0] = static_cast<GLfloat>((ctx->VertexProgram.Current ? ctx->VertexProgram.Current->Id : 0));
	    break;
	case GL_VERTEX_ATTRIB_ARRAY0_NV:
	    CHECK_EXT1(NV_vertex_program, "GetFloatv");
	...
	case GL_FRAGMENT_PROGRAM_BINDING_NV:
	    CHECK_EXT1(NV_fragment_program, "GetFloatv");
	    params[0] = static_cast<GLfloat>(ctx->FragmentProgram.Current ? ctx->FragmentProgram.Current->Id : 0);
	    break;
	case GL_MAX_FRAGMENT_PROGRAM_LOCAL_PARAMETERS_NV:
	    CHECK_EXT1(NV_fragment_program, "GetFloatv");
	    params[0] = static_cast<GLfloat>(MAX_NV_FRAGMENT_PROGRAM_PARAMS);
	    break;
	case GL_TEXTURE_RECTANGLE_NV:
	...
	case GL_TEXTURE_BINDING_RECTANGLE_NV:
	    CHECK_EXT1(NV_texture_rectangle, "GetFloatv");
	    params[0] = static_cast<GLfloat>(ctx->Texture.Unit[ctx->Texture.CurrentUnit].CurrentRect->Name);
	    break;
	case GL_MAX_RECTANGLE_TEXTURE_SIZE_NV:
	    CHECK_EXT1(NV_texture_rectangle, "GetFloatv");
	    params[0] = static_cast<GLfloat>(ctx->Const.MaxTextureRectSize);
	    break;
	case GL_STENCIL_TEST_TWO_SIDE_EXT:
	...
	case GL_ARRAY_BUFFER_BINDING_ARB:
	    ...
	    params[0] = static_cast<GLfloat>(ctx->Array.ArrayBufferObj->Name);
	    break;
	case GL_VERTEX_ARRAY_BUFFER_BINDING_ARB:
	    CHECK_EXT1(ARB_vertex_buffer_object, "GetFloatv");
	    params[0] = static_cast<GLfloat>(ctx->Array.ArrayObj->Vertex.BufferObj->Name);
	    break;
	case GL_NORMAL_ARRAY_BUFFER_BINDING_ARB:
	    CHECK_EXT1(ARB_vertex_buffer_object, "GetFloatv");
	    params[0] = static_cast<GLfloat>(ctx->Array.ArrayObj->Normal.BufferObj->Name);
	    break;
	case GL_COLOR_ARRAY_BUFFER_BINDING_ARB:
	    CHECK_EXT1(ARB_vertex_buffer_object, "GetFloatv");
	    params[0] = static_cast<GLfloat>(ctx->Array.ArrayObj->Color.BufferObj->Name);
	    break;
	case GL_INDEX_ARRAY_BUFFER_BINDING_ARB:
	    CHECK_EXT1(ARB_vertex_buffer_object, "GetFloatv");
	    params[0] = static_cast<GLfloat>(ctx->Array.ArrayObj->Index.BufferObj->Name);
	    break;
	case GL_TEXTURE_COORD_ARRAY_BUFFER_BINDING_ARB:
	    CHECK_EXT1(ARB_vertex_buffer_object, "GetFloatv");
	    params[0] = static_cast<GLfloat>(ctx->Array.ArrayObj->TexCoord[ctx->Array.ActiveTexture].BufferObj->Name);
	    break;
	case GL_EDGE_FLAG_ARRAY_BUFFER_BINDING_ARB:
	    CHECK_EXT1(ARB_vertex_buffer_object, "GetFloatv");
	    params[0] = static_cast<GLfloat>(ctx->Array.ArrayObj->EdgeFlag.BufferObj->Name);
	    break;
	case GL_SECONDARY_COLOR_ARRAY_BUFFER_BINDING_ARB:
	    CHECK_EXT1(ARB_vertex_buffer_object, "GetFloatv");
	    params[0] = static_cast<GLfloat>(ctx->Array.ArrayObj->SecondaryColor.BufferObj->Name);
	    break;
	case GL_FOG_COORDINATE_ARRAY_BUFFER_BINDING_ARB:
	    CHECK_EXT1(ARB_vertex_buffer_object, "GetFloatv");
	    params[0] = static_cast<GLfloat>(ctx->Array.ArrayObj->FogCoord.BufferObj->Name);
	    break;
	case GL_ELEMENT_ARRAY_BUFFER_BINDING_ARB:
	    CHECK_EXT1(ARB_vertex_buffer_object, "GetFloatv");
	    params[0] = static_cast<GLfloat>(ctx->Array.ElementArrayBufferObj->Name);
	    break;
	case GL_PIXEL_PACK_BUFFER_BINDING_EXT:
	    CHECK_EXT1(EXT_pixel_buffer_object, "GetFloatv");
	    params[0] = static_cast<GLfloat>(ctx->Pack.BufferObj->Name);
	    break;
	case GL_PIXEL_UNPACK_BUFFER_BINDING_EXT:
	    CHECK_EXT1(EXT_pixel_buffer_object, "GetFloatv");
	    params[0] = static_cast<GLfloat>(ctx->Unpack.BufferObj->Name);
	    break;
	case GL_VERTEX_PROGRAM_ARB:
	...
	case GL_MAX_PROGRAM_MATRIX_STACK_DEPTH_ARB:
	    ...
	    params[0] = static_cast<GLfloat>(ctx->Const.MaxProgramMatrixStackDepth);
	    break;
	case GL_MAX_PROGRAM_MATRICES_ARB:
	    CHECK_EXT3(ARB_vertex_program, ARB_fragment_program, NV_vertex_program, "GetFloatv");
	    params[0] = static_cast<GLfloat>(ctx->Const.MaxProgramMatrices);
	    break;
	case GL_CURRENT_MATRIX_STACK_DEPTH_ARB:
	    CHECK_EXT3(ARB_vertex_program, ARB_fragment_program, NV_vertex_program, "GetFloatv");
	...
	case GL_MAX_VERTEX_ATTRIBS_ARB:
	    ...
	    params[0] = static_cast<GLfloat>(ctx->Const.VertexProgram.MaxAttribs);
	    break;
	case GL_PROGRAM_ERROR_POSITION_ARB:
	    CHECK_EXT4(NV_vertex_program, ARB_vertex_program, NV_fragment_program, ARB_fragment_program, "GetFloatv");
	    params[0] = static_cast<GLfloat>(ctx->Program.ErrorPos);
	    break;
	case GL_FRAGMENT_PROGRAM_ARB:
	...
	case GL_MAX_TEXTURE_COORDS_ARB:
	    ...
	    params[0] = static_cast<GLfloat>(ctx->Const.MaxTextureCoordUnits);
	    break;
	case GL_MAX_TEXTURE_IMAGE_UNITS_ARB:
	    CHECK_EXT2(ARB_fragment_program, NV_fragment_program, "GetFloatv");
	    params[0] = static_cast<GLfloat>(ctx->Const.MaxTextureImageUnits);
	    break;
	case GL_DEPTH_BOUNDS_TEST_EXT:
	...
	case GL_FRAGMENT_PROGRAM_POSITION_MESA:
	    CHECK_EXT1(MESA_program_debug, "GetFloatv");
	    params[0] = static_cast<GLfloat>(ctx->FragmentProgram.CurrentPosition);
	    break;
	case GL_VERTEX_PROGRAM_POSITION_MESA:
	    CHECK_EXT1(MESA_program_debug, "GetFloatv");
	    params[0] = static_cast<GLfloat>(ctx->VertexProgram.CurrentPosition);
	    break;
	case GL_MAX_DRAW_BUFFERS_ARB:
	    CHECK_EXT1(ARB_draw_buffers, "GetFloatv");
	    params[0] = static_cast<GLfloat>(ctx->Const.MaxDrawBuffers);
	    break;
	case GL_DRAW_BUFFER0_ARB:
	    CHECK_EXT1(ARB_draw_buffers, "GetFloatv");
	    params[0] = ENUM_TO_FLOAT(ctx->DrawBuffer->ColorDrawBuffer[0]);
	...
	case GL_IMPLEMENTATION_COLOR_READ_TYPE_OES:
	    ...
	    params[0] = static_cast<GLfloat>(ctx->Const.ColorReadType);
	    break;
	case GL_IMPLEMENTATION_COLOR_READ_FORMAT_OES:
	    CHECK_EXT1(OES_read_format, "GetFloatv");
	    params[0] = static_cast<GLfloat>(ctx->Const.ColorReadFormat);
	    break;
	case GL_NUM_FRAGMENT_REGISTERS_ATI:
	    CHECK_EXT1(ATI_fragment_shader, "GetFloatv");
	    params[0] = static_cast<GLfloat>(6);
	    break;
	case GL_NUM_FRAGMENT_CONSTANTS_ATI:
	    CHECK_EXT1(ATI_fragment_shader, "GetFloatv");
	    params[0] = static_cast<GLfloat>(8);
	    break;
	case GL_NUM_PASSES_ATI:
	    CHECK_EXT1(ATI_fragment_shader, "GetFloatv");
	    params[0] = static_cast<GLfloat>(2);
	    break;
	case GL_NUM_INSTRUCTIONS_PER_PASS_ATI:
	    CHECK_EXT1(ATI_fragment_shader, "GetFloatv");
	    params[0] = static_cast<GLfloat>(8);
	...
	case GL_NUM_INSTRUCTIONS_TOTAL_ATI:
	    CHECK_EXT1(ATI_fragment_shader, "GetFloatv");
	    params[0] = static_cast<GLfloat>(16);
	    break;
	case GL_COLOR_ALPHA_PAIRING_ATI:
	    CHECK_EXT1(ATI_fragment_shader, "GetFloatv");
	...
	case GL_NUM_LOOPBACK_COMPONENTS_ATI:
	    CHECK_EXT1(ATI_fragment_shader, "GetFloatv");
	    params[0] = static_cast<GLfloat>(3);
	    break;
	case GL_NUM_INPUT_INTERPOLATOR_COMPONENTS_ATI:
	    CHECK_EXT1(ATI_fragment_shader, "GetFloatv");
	    params[0] = static_cast<GLfloat>(3);
	...
	case GL_STENCIL_BACK_FUNC:
	    ...
	    break;
	case GL_STENCIL_BACK_VALUE_MASK:
	    params[0] = static_cast<GLfloat>(ctx->Stencil.ValueMask[1]);
	    break;
	case GL_STENCIL_BACK_WRITEMASK:
	    params[0] = static_cast<GLfloat>(ctx->Stencil.WriteMask[1]);
	    break;
	case GL_STENCIL_BACK_REF:
	    params[0] = static_cast<GLfloat>(ctx->Stencil.Ref[1]);
	    break;
	case GL_STENCIL_BACK_FAIL:
	...
	case GL_FRAMEBUFFER_BINDING_EXT:
	    ...
	    params[0] = static_cast<GLfloat>(ctx->DrawBuffer->Name);
	    break;
	case GL_RENDERBUFFER_BINDING_EXT:
	    CHECK_EXT1(EXT_framebuffer_object, "GetFloatv");
	    params[0] = static_cast<GLfloat>(ctx->CurrentRenderbuffer ? ctx->CurrentRenderbuffer->Name : 0);
	    break;
	case GL_MAX_COLOR_ATTACHMENTS_EXT:
	    CHECK_EXT1(EXT_framebuffer_object, "GetFloatv");
	    params[0] = static_cast<GLfloat>(ctx->Const.MaxColorAttachments);
	    break;
	case GL_MAX_RENDERBUFFER_SIZE_EXT:
	    CHECK_EXT1(EXT_framebuffer_object, "GetFloatv");
	    params[0] = static_cast<GLfloat>(ctx->Const.MaxRenderbufferSize);
	    break;
	case GL_READ_FRAMEBUFFER_BINDING_EXT:
	    CHECK_EXT1(EXT_framebuffer_blit, "GetFloatv");
	    params[0] = static_cast<GLfloat>(ctx->ReadBuffer->Name);
	    break;
	case GL_MAX_FRAGMENT_UNIFORM_COMPONENTS_ARB:
	    CHECK_EXT1(ARB_fragment_shader, "GetFloatv");
	    params[0] = static_cast<GLfloat>(ctx->Const.FragmentProgram.MaxUniformComponents);
	    break;
	case GL_FRAGMENT_SHADER_DERIVATIVE_HINT_ARB:
	    CHECK_EXT1(ARB_fragment_shader, "GetFloatv");
	...
	case GL_MAX_VERTEX_UNIFORM_COMPONENTS_ARB:
	    ...
	    params[0] = static_cast<GLfloat>(ctx->Const.VertexProgram.MaxUniformComponents);
	    break;
	case GL_MAX_VARYING_FLOATS_ARB:
	    CHECK_EXT1(ARB_vertex_shader, "GetFloatv");
	    params[0] = static_cast<GLfloat>(ctx->Const.MaxVarying * 4);
	    break;
	case GL_MAX_VERTEX_TEXTURE_IMAGE_UNITS_ARB:
	    CHECK_EXT1(ARB_vertex_shader, "GetFloatv");
	    params[0] = static_cast<GLfloat>(ctx->Const.MaxVertexTextureImageUnits);
	    break;
	case GL_MAX_COMBINED_TEXTURE_IMAGE_UNITS_ARB:
	    CHECK_EXT1(ARB_vertex_shader, "GetFloatv");
	    params[0] = static_cast<GLfloat>(MAX_COMBINED_TEXTURE_IMAGE_UNITS);
	    break;
	case GL_CURRENT_PROGRAM:
	    CHECK_EXT1(ARB_shader_objects, "GetFloatv");
	    params[0] = static_cast<GLfloat>(ctx->Shader.CurrentProgram ? ctx->Shader.CurrentProgram->Name : 0);
	    break;
	default:
...
_mesa_GetIntegerv(GLenum pname, GLint *params)
{
	...
	case GL_VERTEX_PROGRAM_BINDING_NV:
	    ...
	    params[0] = (ctx->VertexProgram.Current ? ctx->VertexProgram.Current->Id : 0);
	    break;
	case GL_VERTEX_ATTRIB_ARRAY0_NV:
	    CHECK_EXT1(NV_vertex_program, "GetIntegerv");
	...
	case GL_FRAGMENT_PROGRAM_BINDING_NV:
	    CHECK_EXT1(NV_fragment_program, "GetIntegerv");
	    params[0] = ctx->FragmentProgram.Current ? ctx->FragmentProgram.Current->Id : 0;
	    break;
	case GL_MAX_FRAGMENT_PROGRAM_LOCAL_PARAMETERS_NV:
	    CHECK_EXT1(NV_fragment_program, "GetIntegerv");
